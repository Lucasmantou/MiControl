...
//!
//! ```text
//! cargo build -p micontrol-windows --example installer_exit_mock
//! copy target\debug\examples\installer_exit_mock.exe <任意目录>\micontrol-windows-app.exe
//! <任意目录>\micontrol-windows-app.exe      # 阻塞，直到收到退出信号
//! ```
//!
//! 然后运行安装器：钩子应检测到进程、置位事件、等到进程退出并继续安装，
//! **既不弹 Tauri 的"终止运行"对话框，也不强杀进程**。

use micontrol_windows::graceful_exit::{GracefulExitSignal, GRACEFUL_EXIT_EVENT_NAME};

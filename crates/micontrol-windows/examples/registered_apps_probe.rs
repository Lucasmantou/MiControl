fn main() {
    let apps =
        micontrol_windows::registered_apps::scan_registered_apps().expect("registered app scan");
    ...
    if let Some(name) = std::env::args().nth(1) {
        ...
        micontrol_windows::app_launcher::activate_or_launch(&app.path).expect("registered launch");

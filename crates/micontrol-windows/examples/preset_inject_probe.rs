...
//! 人工确认。
//!
//! 运行：`cargo run -p micontrol-windows --release --example preset_inject_probe`

#[cfg(windows)]
mod windows_impl {
    use micontrol_windows::app_launcher;
    use micontrol_windows::send_input::{plan_key_tap, KeyChord, KeyCode};
    use micontrol_windows::send_input_windows::SendInputRuntime;

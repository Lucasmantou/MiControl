use micontrol_windows::{gatt_note, initialize_diagnostic_log, DiagnosticLogMetadata};

#[test]
fn production_default_log_path_writes_structured_metadata_without_environment_override() {
    let path = std::env::temp_dir().join(format!(
        "micontrol-production-log-test-{}.log",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    // SAFETY: 集成测试拥有独立进程，只在初始化 OnceLock 前修改本进程环境。
    unsafe { std::env::remove_var("MICONTROL_GATT_LOG") };

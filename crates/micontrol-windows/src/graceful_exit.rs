...
//! 必须与 `src-tauri/windows/installer-hooks.nsh` 的 `MICONTROL_GRACEFUL_EXIT_EVENT`
//! 同步。安装器侧的等待是"先静默 `MICONTROL_GRACEFUL_EXIT_SETTLE_MS`（1500ms），
//! 若进程仍在再补 `MICONTROL_GRACEFUL_EXIT_TAIL_MS`（6500ms）"，合计 8 秒；
//! 应用侧的收尾超时必须明显小于它。
...
pub const GRACEFUL_EXIT_EVENT_NAME: &str = r"Local\MiControl-GracefulExit";
...
mod tests {
    ...
    fn event_name_matches_the_installer_contract() {
        assert_eq!(GRACEFUL_EXIT_EVENT_NAME, r"Local\MiControl-GracefulExit");
    }

    #[test]
    fn bounded_wait_times_out_without_a_signal() {
        let signal = GracefulExitSignal::create_named(r"Local\MiControl-GracefulExit-test-timeout")
    ...
    fn signalling_an_absent_event_fails_so_the_installer_can_skip() {
        assert!(signal_named(r"Local\MiControl-GracefulExit-absent").is_err());
    }

    #[test]
    fn signal_is_observed_by_a_waiter() {
        const NAME: &str = r"Local\MiControl-GracefulExit-test-signal";

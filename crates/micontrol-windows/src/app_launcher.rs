...
pub const PRESET_APPS: &[PresetApp] = &[
    PresetApp {
        id: "micontrol",
        name: "MiControl",
        exe_names: &["micontrol-windows-app.exe"],
    },
    PresetApp {
        id: "wechat",
        name: "微信",
...
pub fn probe_preset_apps() -> Vec<PresetAppInfo> {
    PRESET_APPS
        ...
        .map(|app| PresetAppInfo {
            ...
            installed: app.id == "micontrol" || app.exe_names.iter().any(|exe| exe_resolvable(exe)),
        })
        .collect()
}
...
fn resolve_lnk(lnk_path: &str) -> Option<ResolvedShortcut> {
    ...
    let handle = std::thread::Builder::new()
        .name("micontrol-resolve-lnk".to_owned())
        .spawn(move || {
...
mod win_impl {
    ...
    pub(super) unsafe fn show_and_force_foreground(hwnd: HWND) {
        ...
        // 脱节，因此调用方（`activate_or_launch` 的 micontrol 分支）必须随后调用
...
fn launch_new(exe_names: &[&str]) -> Result<(), String> {
    ...
    let handle = std::thread::Builder::new()
        .name("micontrol-app-launch".to_owned())
        .spawn(move || {
            use windows::core::PCWSTR;
...
fn launch_explicit(
    ...
    let handle = std::thread::Builder::new()
        .name("micontrol-app-launch-path".to_owned())
        .spawn(move || {
            use windows::core::PCWSTR;
            use windows::Win32::UI::Shell::ShellExecuteW;
            use windows::Win32::UI::WindowsAndMessaging::SW_SHOWNORMAL;
...
pub fn pick_custom_app() -> Option<CustomAppPick> {
    let handle = std::thread::Builder::new()
        .name("micontrol-pick-app".to_owned())
        .spawn(|| {
            use windows::core::PCWSTR;
            use windows::Win32::System::Com::{
...
mod tests {
    ...
    fn lnk_resolution_round_trips() {
        ...
        let lnk_path = std::env::temp_dir().join("micontrol-lnk-roundtrip-test.lnk");
        let lnk = lnk_path.to_string_lossy().to_string();
        let created = std::thread::Builder::new()
            .name("micontrol-lnk-create".to_owned())
            .spawn(move || {
                use windows::core::{Interface, PCWSTR};
    ...
    #[test]
    #[cfg(windows)]
    fn micontrol_preset_is_always_installed() {
    ...
    fn file_dialog_com_pipeline_is_usable() {
        let ok = std::thread::Builder::new()
            .name("micontrol-dialog-probe".to_owned())
            .spawn(|| {
                use windows::core::PCWSTR;
                use windows::Win32::System::Com::{
                    CoCreateInstance, CoInitializeEx, CoUninitialize, CLSCTX_INPROC_SERVER,
    ...
    fn open_directory_rejects_missing_directory() {
        let missing = std::env::temp_dir().join("micontrol-open-directory-missing-probe");
    ...
    ///
    /// 运行：`cargo test -p micontrol-windows --lib -- --ignored open_directory_opens_explorer`
    #[test]
    #[cfg(windows)]
    #[ignore = "会真实打开资源管理器窗口，仅在需要取证时手动运行"]
    fn open_directory_opens_explorer() {
        let directory = std::env::temp_dir().join("micontrol-open-directory-probe");

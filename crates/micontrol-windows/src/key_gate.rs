...
//!      2026-09-06 纳入）物理键盘实际极罕见。三者无需武装直接吞（结构性武装死锁：孤立按压首沿在 60ms 有界等待内无法武装必泄漏）；
    ...
    impl KeyGate {
        pub fn start() -> KeyGate {
            ...
            let worker = std::thread::Builder::new()
                .name("micontrol-key-gate".to_owned())

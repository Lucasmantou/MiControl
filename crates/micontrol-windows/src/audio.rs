...
impl AudioRuntime {
    pub fn new() -> Self {
        ...
        let worker = thread::Builder::new()
            .name("micontrol-wasapi".to_owned())
    ...
    fn open(endpoint_id: &str, attempt_id: u64) -> Result<Self, PlatformError> {
        ...
        if is_virtual_cable {
            client
                ...
                .map_err(|error| audio_error("关闭 MiControl 会话的系统通信自动静音", error))?;
            crate::ble::gatt_note(
    ...
    fn ensure_session_unmuted(
        ...
        if self.session_volumes.is_empty() {
            self.session_volumes = process_audio_session_volumes(&self.endpoint_id)
                .map_err(|error| audio_error("查找 MiControl 音频会话", error))?;
        }
        if self.session_volumes.is_empty() {
            crate::ble::gatt_note(format!(
        ...
        for volume in &self.session_volumes {
            let was_muted = unsafe { volume.GetMute() }
                .map_err(|error| audio_error("读取 MiControl 会话静音状态", error))?
                .as_bool();
            minimum_level = minimum_level.min(
                unsafe { volume.GetMasterVolume() }
                    .map_err(|error| audio_error("读取 MiControl 会话音量", error))?,
            );
            if was_muted {
                muted_before += 1;
                unsafe { volume.SetMute(false, std::ptr::null()) }
                    .map_err(|error| audio_error("解除 MiControl 会话静音", error))?;
            }
            if unsafe { volume.GetMute() }
                .map_err(|error| audio_error("确认 MiControl 会话静音状态", error))?
                .as_bool()
            {
                muted_after += 1;
            }
        ...
        if muted_after > 0 {
            return Err(PlatformError::Audio(format!(
                "MiControl 音频会话解除静音后仍有 {muted_after} 个会话处于静音"
            )));
        }
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    #[ignore = "requires MICONTROL_TEST_CABLE_ENDPOINT_ID and mutates that endpoint's mute state"]
    fn cable_endpoint_unmutes_on_open_and_each_session() {
        let endpoint_id = std::env::var("MICONTROL_TEST_CABLE_ENDPOINT_ID")
            .expect("set MICONTROL_TEST_CABLE_ENDPOINT_ID to the CABLE Input render endpoint");
        ...
        let session_volumes = process_audio_session_volumes(&endpoint_id)
            .expect("enumerate the initialized MiControl test session");
                ...
                .map(|volume| {
                    (
                        ...
                        unsafe { volume.GetMute() }
                            .expect("read original MiControl test session mute")
                            .as_bool(),
                    )
                })
                .collect(),
        ...
        assert!(
            set_process_sessions_muted(&endpoint_id, true)
                .expect("mute MiControl session before begin")
                > 0,
        ...
        assert!(
            ...
            "begin_session should unmute the MiControl session"
        );

        set_process_sessions_muted(&endpoint_id, true)
            .expect("mute MiControl session before stream start");
        runtime
        ...
        assert!(
            ...
            "starting the stream should unmute the MiControl session again"
        );

        set_process_sessions_muted(&endpoint_id, true)
            .expect("mute MiControl session while streaming");

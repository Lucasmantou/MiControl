...
use micontrol_core::{AtvvCommand, AtvvVoicePipeline, PipelineOutput, VoiceSessionState};
...
pub fn initialize_diagnostic_log(
    ...
    let path = std::env::var_os("MICONTROL_GATT_LOG")
...
pub fn diagnostic_log_directory() -> Option<std::path::PathBuf> {
    DIAGNOSTIC_LOG_PATH
        .get()
        .cloned()
        .or_else(|| std::env::var_os("MICONTROL_GATT_LOG").map(std::path::PathBuf::from))?
        .parent()
        .map(std::path::Path::to_path_buf)
}

/// ATVV 诊断日志（宿主默认写入 LocalAppData；MICONTROL_GATT_LOG 可覆盖路径）。
...
fn spawn_wetype_check(
    ...
    std::thread::Builder::new()
        .name("micontrol-wetype-check".to_owned())
        .spawn(move || {

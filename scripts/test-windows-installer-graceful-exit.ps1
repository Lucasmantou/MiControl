...
$diagnosticLogPath = Join-Path $env:LOCALAPPDATA "MiControl/Logs/micontrol-diagnostic.log"

# 与 crates/micontrol-windows/src/graceful_exit.rs 的 GRACEFUL_EXIT_EVENT_NAME 一致；
# src-tauri 的契约测试断言它与 installer-hooks.nsh 中的定义逐字相同。
$gracefulExitEventName = "Local\MiControl-GracefulExit"
...
function Invoke-SilentUninstall($entry) {
    ...
    if (-not (Wait-ProcessExit $process 180)) {
        throw "Silent MiControl uninstall did not finish in time"
    }
...
if (@(Get-MiControlUninstallEntries).Count -ne 0) {
    throw "A MiControl installation already exists before the installer graceful-exit test"
}
...
try {
    ...
    if ($installProcess.ExitCode -ne 0) {
        throw "Silent MiControl install failed with exit code $($installProcess.ExitCode)"
    }
    $installEntries = @(Wait-SingleInstallation)
    if ($installEntries.Count -ne 1) {
        throw "Expected one MiControl uninstall entry after install, found $($installEntries.Count)"
    }
    ...
    if ($afterInstallEntries.Count -ne 1) {
        throw "Expected one MiControl uninstall entry after over-install, found $($afterInstallEntries.Count)"
    }
    ...
    while (@(Get-MiControlUninstallEntries).Count -ne 0 -and [DateTime]::UtcNow -lt $deadline) {
        Start-Sleep -Milliseconds 250
    }
    if (@(Get-MiControlUninstallEntries).Count -ne 0) {
        throw "MiControl uninstall entry remains after the graceful-exit test"
    }

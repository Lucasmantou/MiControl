...
$appExecutable = Join-Path $repositoryRoot "target/release/micontrol-windows-app.exe"
...
$reportPath = Join-Path $reportDirectory "micontrol-runtime-simulation-$simulationId.json"
$stateDirectory = Join-Path $reportDirectory "micontrol-runtime-simulation-state-$simulationId"
$stdoutPath = Join-Path $reportDirectory "micontrol-runtime-simulation-$simulationId.stdout.log"
$stderrPath = Join-Path $reportDirectory "micontrol-runtime-simulation-$simulationId.stderr.log"
$env:MICONTROL_WINDOWS_RUNTIME_SIMULATION = "1"
$env:MICONTROL_RUNTIME_SIMULATION_REPORT = $reportPath
$env:MICONTROL_RUNTIME_SIMULATION_STATE_DIR = $stateDirectory
$appProcess = $null

...
$compatibilitySourcePath = Join-Path $repositoryRoot "crates/micontrol-windows/src/compatibility.rs"
...
$applicationPath = Join-Path $repositoryRoot "target/release/micontrol-windows-app.exe"

$config = Get-Content -Raw -Encoding UTF8 $configPath | ConvertFrom-Json
if ($config.productName -ne "MiControl") {
    throw "Unexpected productName: $($config.productName)"
}
if ($config.identifier -ne "app.micontrol.windows") {
    throw "Unexpected application identifier: $($config.identifier)"
}
if ($config.bundle.publisher -ne "MiControl") {
...
if ($installerHooks -notmatch '(?m)^!define MICONTROL_MINIMUM_WINDOWS_BUILD 17763\r?$') {
...
if (
    $installerHooks -notmatch '(?m)^!define MICONTROL_DOWNGRADE_ERROR_LEVEL 1638\r?$' -or
    ...
    $installerHooks -notmatch 'SetErrorLevel \$\{MICONTROL_DOWNGRADE_ERROR_LEVEL\}'
) {

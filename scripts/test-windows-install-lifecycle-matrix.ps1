...
function Get-SingleInstallation([Version] $expectedVersion) {
    ...
    if ($entries.Count -ne 1) {
        throw "Expected one MiControl uninstall entry, found $($entries.Count)"
    }
...
function Assert-SingleShortcut {
    ...
    if ($shortcuts.Count -ne 1 -or $shortcuts[0].Name -ne "$productName.lnk") {
        throw "Expected exactly one MiControl Start Menu shortcut"
    }
    $directory
}

$currentInstallers = @(Get-ChildItem -LiteralPath $bundleDirectory -Filter "*-setup.exe" -File)
$predecessorInstallers = @(Get-ChildItem -LiteralPath $fixtureDirectory -Filter "micontrol-predecessor-*-setup.exe" -File)
...
if ($predecessorInstaller.BaseName -notmatch '^micontrol-predecessor-(?<version>\d+\.\d+\.\d+)-setup$') {
...
if (@(Get-MiControlUninstallEntries).Count -ne 0) {
    throw "A MiControl installation already exists before the lifecycle matrix"
}
...
try {
    ...
    do {
        $upgradeEntries = @(Get-MiControlUninstallEntries)
    ...
    if (@(Get-MiControlUninstallEntries).Count -ne 0) { throw "Uninstall entry remains after matrix uninstall" }
...
"@ | Add-Content -Encoding UTF8 $env:GITHUB_STEP_SUMMARY
    ...
    if (-not $normalUninstallCompleted) {
        foreach ($entry in @(Get-MiControlUninstallEntries)) {

...
function Get-MiControlUninstallEntries {
    $uninstallRoots = @(
...
function Invoke-SilentUninstall($entry) {
    ...
    if ([string]::IsNullOrWhiteSpace($command)) {
        throw "MiControl uninstall registry entry has no uninstall command"
    }

    $parts = Split-ExecutableCommand $command
    if (-not (Test-Path -LiteralPath $parts.FilePath -PathType Leaf)) {
        throw "MiControl uninstaller is missing: $($parts.FilePath)"
    }
    ...
    if ($process.ExitCode -ne 0) {
        throw "Silent MiControl uninstall failed with exit code $($process.ExitCode)"
    }
}
...
if (@(Get-MiControlUninstallEntries).Count -ne 0) {
    throw "A MiControl installation already exists before the CI lifecycle test"
}

try {
    $installerProcess = Start-Process -FilePath $installers[0].FullName -ArgumentList "/S" -Wait -PassThru
    if ($installerProcess.ExitCode -ne 0) {
        throw "Silent MiControl install failed with exit code $($installerProcess.ExitCode)"
    }

    $entries = @(Get-MiControlUninstallEntries)
    if ($entries.Count -ne 1) {
        throw "Expected one current-user MiControl uninstall entry after install, found $($entries.Count)"
    }
    $entry = $entries[0]
    ...
    if ([string]::IsNullOrWhiteSpace($registeredInstallLocation)) {
        throw "MiControl uninstall entry has no InstallLocation"
    }
    if ($installedVersion -ne $config.version) {
        throw "Installed MiControl version $installedVersion does not match $($config.version)"
    }
    if (
        ...
        throw "MiControl uninstall entry has an invalid MainBinaryName: $mainBinaryName"
    }
    ...
    if (-not (Test-Path -LiteralPath $installLocation -PathType Container)) {
        throw "Installed MiControl directory is missing: $installLocation"
    }
    ...
    if (-not (Test-Path -LiteralPath $startMenuDirectory -PathType Container)) {
        throw "MiControl Start Menu folder is missing: $startMenuDirectory"
    }
    $shortcuts = @(Get-ChildItem -LiteralPath $startMenuDirectory -Filter "*.lnk" -File -Recurse)
    if ($shortcuts.Count -ne 1) {
        throw "Expected one MiControl Start Menu shortcut, found $($shortcuts.Count)"
    }
    if ($shortcuts[0].Name -ne "$productName.lnk") {
        throw "Unexpected MiControl Start Menu shortcut name: $($shortcuts[0].Name)"
    }
    $appExecutable = Join-Path $installLocation $mainBinaryName
    if (-not (Test-Path -LiteralPath $appExecutable -PathType Leaf)) {
        throw "Installed MiControl executable is missing: $appExecutable"
    }
    ...
    if ($appProcess.HasExited) {
        throw "Installed MiControl exited during the 8-second launch smoke test with code $($appProcess.ExitCode)"
    }
    ...
    if (@(Get-MiControlUninstallEntries).Count -ne 0) {
        throw "MiControl uninstall registry entry remains after silent uninstall"
    }
    if (Test-Path -LiteralPath $appExecutable) {
        throw "MiControl executable remains after silent uninstall: $appExecutable"
    }
    if (Test-Path -LiteralPath $installLocation) {
        throw "MiControl install directory remains after silent uninstall: $installLocation"
    }
    if (Test-Path -LiteralPath $startMenuDirectory) {
        throw "MiControl Start Menu folder remains after silent uninstall: $startMenuDirectory"
    }
    if (-not (Test-Path -LiteralPath $preservationMarker -PathType Leaf)) {
        throw "Silent uninstall unexpectedly removed the MiControl app-config marker"
    }
...
"@ | Add-Content -Encoding UTF8 $env:GITHUB_STEP_SUMMARY
    ...
    if (-not $normalUninstallCompleted) {
        foreach ($remainingEntry in @(Get-MiControlUninstallEntries)) {
            try {

use micontrol_windows::button_mapping::{ButtonEdgeCallback, ButtonGestureCallback};
use micontrol_windows::raw_input::{RawInputSnapshot, RemoteButton};
use micontrol_windows::send_input::{
    ButtonAction, ButtonMappings, ButtonTrigger, KeyChord, SendInputSnapshot,
};
use micontrol_windows::{
...
use micontrol_core::ThemePreference;
...
async fn save_button_mappings(
    ...
    micontrol_windows::gatt_note(format!(
    ...
    micontrol_windows::gatt_note(match &result {
        Ok(saved) => format!(
...
async fn export_button_mapping_configuration(
    ...
    let result = match tauri::async_runtime::spawn_blocking(move || -> Result<bool, String> {
        let Some(path) = micontrol_windows::file_dialog::pick_button_mapping_export_path()? else {
            return Ok(false);
        };
...
async fn import_button_mapping_configuration(
    ...
    let result = match tauri::async_runtime::spawn_blocking(
        move || -> Result<Option<ButtonMappings>, String> {
            let Some(path) = micontrol_windows::file_dialog::pick_button_mapping_import_path()? else {
                return Ok(None);
            };
...
fn list_preset_apps(
    state: tauri::State<'_, AppState>,
) -> Vec<micontrol_windows::app_launcher::PresetAppInfo> {
    state.platform.preset_apps()
}

/// 原生文件选择器：选择自定义应用（.exe/.lnk）。用户取消返回 null。
#[tauri::command]
fn pick_custom_app() -> Option<micontrol_windows::app_launcher::CustomAppPick> {
    micontrol_windows::app_launcher::pick_custom_app()
}

#[tauri::command]
async fn scan_registered_apps() -> Result<Vec<micontrol_windows::app_launcher::CustomAppPick>, String>
{
    tauri::async_runtime::spawn_blocking(micontrol_windows::registered_apps::scan_registered_apps)
        .await
...
fn get_button_mapping_snapshot(
    state: tauri::State<'_, AppState>,
) -> micontrol_windows::button_mapping::ButtonMappingSnapshot {
...
fn start_shortcut_capture() -> Result<(), String> {
    ...
    if !micontrol_windows::key_gate::set_shortcut_capture_active(true) {
        micontrol_windows::gatt_note(format!(
...
fn stop_shortcut_capture() {
    let _ = micontrol_windows::key_gate::set_shortcut_capture_active(false);
    micontrol_windows::gatt_note(
...
async fn get_theme_preference(
    ...
    match &result {
        Ok(preference) => micontrol_windows::gatt_note(format!(
        ...
        Err(_) => micontrol_windows::gatt_note(format!(
...
async fn set_theme_preference(
    ...
    match &result {
        Ok(saved) => micontrol_windows::gatt_note(format!(
...
fn complete_runtime_simulation_smoke(
    ...
    let report_path = std::env::var_os("MICONTROL_RUNTIME_SIMULATION_REPORT")
...
fn register_shortcut_capture_events(app: tauri::AppHandle) {
    ...
    micontrol_windows::key_gate::set_shortcut_capture_sink(Arc::new(move |edge| {
        let _ = sender.try_send(edge);
    }));
    std::thread::Builder::new()
        .name("micontrol-shortcut-capture-events".to_owned())
        .spawn(move || {
...
fn spawn_raw_input_supervisor(platform: Arc<dyn PlatformRuntime>) {
    std::thread::Builder::new()
        .name("micontrol-raw-input-supervisor".to_owned())
        .spawn(move || {
            let mut initial_attempt_pending = true;
            loop {
                let phase = platform.raw_input_snapshot().phase;
                let should_start = phase == micontrol_windows::raw_input::RawInputPhase::Failed
                    || (initial_attempt_pending
                        && phase == micontrol_windows::raw_input::RawInputPhase::Stopped);
                if should_start {
...
fn runtime_simulation_requested() -> bool {
    std::env::var_os("MICONTROL_WINDOWS_RUNTIME_SIMULATION").as_deref()
...
/// 同进程其它日志测试钉死，见 `micontrol_windows::gatt_note` 的注释）。
...
fn spawn_installer_graceful_exit_watcher(app: tauri::AppHandle) {
    let spawned = std::thread::Builder::new()
        .name("micontrol-graceful-exit".to_owned())
        .spawn(move || {
            let signal = match micontrol_windows::graceful_exit::GracefulExitSignal::create() {
                Ok(signal) => signal,
                Err(error) => {
                    micontrol_windows::gatt_note(format!(
...
pub fn run() {
    let log_path = std::env::var_os("LOCALAPPDATA")
        ...
        .join("MiControl")
        .join("Logs")
        .join("micontrol-diagnostic.log");
    let log_ready = micontrol_windows::initialize_diagnostic_log(
        log_path,
        micontrol_windows::DiagnosticLogMetadata {
            app_version: env!("CARGO_PKG_VERSION").to_owned(),
            app_build: option_env!("MICONTROL_APP_BUILD")
                .unwrap_or("unknown")
                .to_owned(),
            source_revision: env!("MICONTROL_SOURCE_REVISION").to_owned(),
            build_channel: option_env!("MICONTROL_BUILD_CHANNEL")
                .unwrap_or("unknown")
                .to_owned(),
            release_tag: option_env!("MICONTROL_RELEASE_TAG")
                .unwrap_or("unknown")
                .to_owned(),
        },
    );
    ...
    #[cfg(windows)]
    if let Err(error) = micontrol_windows::compatibility::check_current_windows() {
        micontrol_windows::gatt_note(
        ...
        micontrol_windows::compatibility::show_unsupported_windows_message(error);
        eprintln!("{error}");
        return;
    }
    ...
    // 父对象目录存在（"MiControl\Windows\…" 直接 ERROR_PATH_NOT_FOUND，
    ...
    #[cfg(windows)]
    {
        ...
        const SINGLE_INSTANCE_MUTEX: windows::core::PCWSTR = w!("MiControl.Windows.SingleInstance");
        match unsafe { CreateMutexW(None, false, SINGLE_INSTANCE_MUTEX) } {
            Ok(handle) => {
                ...
                if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS {
                    ...
                    eprintln!("MiControl 已在运行：单实例守卫阻止了第二个实例启动");
                    unsafe {
    ...
    let builder = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(all(windows, not(feature = "runtime-simulation")))]
            {
                ...
                TrayIconBuilder::with_id("micontrol-tray")
                    .icon(icon)
                    .menu(&menu)
            ...
            let settings_path = if runtime_simulation_requested() {
                let directory = std::env::var_os("MICONTROL_RUNTIME_SIMULATION_STATE_DIR")
                    .ok_or_else(|| {
                        std::io::Error::new(
            ...
            #[cfg(all(windows, not(feature = "runtime-simulation")))]
            micontrol_windows::prepare_bluetooth_radio_recovery();
            ...
            // "打开 MiControl"（自身窗口）后的 tao 可见性缓存同步：`app_launcher` 用
            ...
            // 幂等无副作用。显示与隐藏同走一条事件队列，FIFO 保证同步在前。
            {
                ...
                micontrol_windows::app_launcher::set_self_show_sync(move || {
    ...
    if let Err(_) = built.map(|app| {
        ...
        panic!("failed to run MiControl Windows app");
    }
    micontrol_windows::gatt_note(
...
mod tests {
    ...
    fn installer_hook_requests_graceful_exit_with_the_app_event_name() {
        let expected = format!(
            "!define MICONTROL_GRACEFUL_EXIT_EVENT \"{}\"",
            micontrol_windows::graceful_exit::GRACEFUL_EXIT_EVENT_NAME
        );
        assert!(
        ...
        let request = macro_body(INSTALLER_HOOKS, "MiControlRequestGracefulExit");
        ...
        for hook in ["NSIS_HOOK_PREINSTALL", "NSIS_HOOK_PREUNINSTALL"] {
            assert!(
                macro_body(INSTALLER_HOOKS, hook)
                    .contains("!insertmacro MiControlRequestGracefulExit"),
                "`{hook}` 没有请求应用优雅退出"
            );
        }
    }
    ...
    fn installer_grace_window_exceeds_the_app_shutdown_budget() {
        let settle = define_number(INSTALLER_HOOKS, "MICONTROL_GRACEFUL_EXIT_SETTLE_MS");
        let max_wait = define_number(INSTALLER_HOOKS, "MICONTROL_GRACEFUL_EXIT_MAX_WAIT_MS");
    ...
    fn installer_hook_polls_until_the_process_is_gone() {
        ...
        assert!(request.contains("micontrol_wait_"), "必须有轮询等待循环");
        // 标签后缀由调用方传入：`${__LINE__}` 在卸载段会展开成复合 token。
        for call in [
            "MiControlRequestGracefulExit install",
            "MiControlRequestGracefulExit uninstall",
        ] {
            assert!(

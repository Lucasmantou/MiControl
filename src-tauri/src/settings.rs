use micontrol_core::{AppSettings, ThemePreference, UsageStatistics};
use micontrol_windows::send_input::{ButtonMappings, KeyChord};
use std::fs;
...
impl SettingsStore {
    ...
    pub fn default_voice_hold_hotkey() -> Option<KeyChord> {
        Some(KeyChord {
            keys: vec![
                micontrol_windows::send_input::KeyCode::LeftControl,
                micontrol_windows::send_input::KeyCode::LeftWindows,
            ],
        })
    }
...
mod tests {
    ...
    fn theme_preference_defaults_to_system_and_persists() {
        let path = std::env::temp_dir().join(format!(
            "micontrol-test-theme-preference-{}.json",
            std::process::id()
        ));
    ...
    fn launch_at_login_defaults_off_and_persists() {
        let path = std::env::temp_dir().join(format!(
            "micontrol-test-launch-at-login-{}.json",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        let store = SettingsStore::new(path.clone());
    ...
    fn button_mapping_json_round_trip_preserves_typed_shortcut() {
        use micontrol_windows::raw_input::RemoteButton;
    ...
    fn exported_button_mapping_configuration_is_stable_versioned_and_rejects_before_mutation() {
        use micontrol_windows::raw_input::RemoteButton;
        use micontrol_windows::send_input::{ButtonAction, ButtonActions, KeyChord, KeyCode};

        let base = std::env::temp_dir().join(format!(
            "micontrol-test-button-mapping-config-{}",
            std::process::id()
        ));
        ...
        mappings
            .applications
            .push(micontrol_windows::app_launcher::CustomAppPick {
                name: "Example".into(),
    ...
    fn voice_hold_hotkey_round_trips_and_validates_chord() {
        let store = SettingsStore::new(std::env::temp_dir().join(format!(
            "micontrol-test-voice-hold-{}.json",
            std::process::id()
        )));
        ...
        let right_alt = KeyChord {
            keys: vec![micontrol_windows::send_input::KeyCode::RightAlt],
        };
        let saved = store

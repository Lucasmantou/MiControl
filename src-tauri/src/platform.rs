use micontrol_windows::raw_input::RawInputSnapshot;
use micontrol_windows::send_input::{ButtonAction, KeyChord, ScrollDirection, SendInputSnapshot};
...
pub trait PlatformRuntime: Debug + Send + Sync {
    ...
    /// 预设应用清单（含安装状态）。
    fn preset_apps(&self) -> Vec<micontrol_windows::app_launcher::PresetAppInfo>;
    ...
    fn button_mappings(&self) -> micontrol_windows::send_input::ButtonMappings;
    fn set_button_mappings(&self, mappings: micontrol_windows::send_input::ButtonMappings);
    fn button_mapping_snapshot(&self) -> micontrol_windows::button_mapping::ButtonMappingSnapshot;
    fn subscribe_button_edges(&self, callback: micontrol_windows::button_mapping::ButtonEdgeCallback);
    fn subscribe_button_gestures(
        &self,
        callback: micontrol_windows::button_mapping::ButtonGestureCallback,
    );
...
impl PlatformRuntime for WindowsPlatform {
    ...
    fn preset_apps(&self) -> Vec<micontrol_windows::app_launcher::PresetAppInfo> {
        micontrol_windows::app_launcher::probe_preset_apps()
    }

    fn launch_app(&self, target: &str) -> Result<(), PlatformError> {
        micontrol_windows::app_launcher::activate_or_launch(target)
...
mod simulation {
    use super::*;
    use micontrol_core::{AtvvCapabilities, AtvvVoicePipeline, PipelineOutput, VoiceSessionState};
    use micontrol_windows::raw_input::{RawInputPhase, RemoteButton};
    use micontrol_windows::send_input::{plan_key_tap, KeyChord};
    use micontrol_windows::{AudioPhase, ConnectionPhase, RemoteModel};
    ...
    pub struct SimulatedPlatform {
        ...
        button_mappings: Mutex<micontrol_windows::send_input::ButtonMappings>,
    }
    ...
    impl PlatformRuntime for SimulatedPlatform {
        ...
        fn test_scroll(
            ...
            micontrol_windows::send_input::validate_mouse_amount(steps, 100)
                .map_err(|error| PlatformError::SendInput(error.to_string()))?;
            let mut state = lock(&self.state);
            state.send_input.submitted_batches =
                state.send_input.submitted_batches.saturating_add(1);
        ...
        fn preset_apps(&self) -> Vec<micontrol_windows::app_launcher::PresetAppInfo> {
            ...
            micontrol_windows::app_launcher::PRESET_APPS
                .iter()
                .map(|app| micontrol_windows::app_launcher::PresetAppInfo {
                    id: app.id.to_owned(),
        ...
        fn subscribe_button_edges(
            &self,
            _callback: micontrol_windows::button_mapping::ButtonEdgeCallback,
        ) {
            // CI 仿真不产生真实按键边沿。
        }
        ...
        fn subscribe_button_gestures(
            &self,
            _callback: micontrol_windows::button_mapping::ButtonGestureCallback,
        ) {
            // CI 仿真不产生真实手势。
        }
    ...
    mod tests {
        use super::*;
        use micontrol_windows::send_input::{KeyChord, KeyCode};
